...
export default function Introduction() {
    ...
    return(
        <div className="flex flex-col items-center xl:items-start justify-start w-full h-auto gap-10 xl:gap-10 px-2 xl:px-0">
            {/* Introduction */}
            <div className="flex flex-col items-center xl:items-start justify-start w-full h-auto gap-4 md:px-50 xl:px-0">
                <h1 className="font-poppins tracking-tight text-center xl:text-start text-[40px] md:text-[70px] xl:text-[100px] leading-none text-[#dde3e6] font-extrabold">SOFTWARE<br/><span className="text-neutral-700">ENGINEER</span></h1>
            ...
            <div className="flex flex-row items-start justify-center xl:justify-start w-full h-auto gap-10">
                {careerInfo.map((info) => (    
                    <div key={info.id} className="flex flex-col items-center xl:items-start justify-start w-35 h-auto">
                        <h2 className="text-[#dde3e6] text-[75px] font-semibold leading-20">{info.value}</h2>
                        <p className="text-neutral-500 text-center xl:text-start text-[15px] font-semibold leading-tight">{info.title}</p>
                    </div>
                ))}
            </div>
            <div className="flex flex-col md:flex-row items-center justify-between w-full xl:w-full md:w-3/4 h-45 md:h-15 p-5 gap-2 md:gap-5 bg-[#dde3e6] rounded-xl">
                ...
                <button className="h-10 md:h-8 w-full md:w-8 border-2 md:rounded-lg p-1 hover:bg-white/50 hover:scale-105 transition-all duration-200 bg-black md:bg-transparent cursor-pointer">
